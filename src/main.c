...
static struct mgos_ssd1306 *myOled;
...
bool myDisplayInit(){
  char* vypis;
  bool ret=0;
  vypis="tohle je na display";
  ret=mgos_ssd1306_init();
  myOled=mgos_ssd1306_get_global();
  mgos_ssd1306_start(myOled);
 // mgos_ssd1306_draw_circle(myOled,25,10,5,SSD1306_COLOR_WHITE);
  // mgos_ssd1306_clear(myOled);
  mgos_ssd1306_clear(myOled);
  mgos_ssd1306_select_font(myOled,1); 
  mgos_ssd1306_draw_string(myOled,3,3,vypis);
  mgos_ssd1306_draw_string(myOled,3,15,vypis);
  return ret;
}


static void myDisplayWritestring(){
  char* vypis;
  vypis="tohle je na display";
  mgos_ssd1306_draw_string(myOled,3,3,vypis);
}

static void myDisplayRefresh(){
mgos_ssd1306_refresh(myOled,0);
}
static void myDisplayRefreshBig(){
  char* vypis;
  vypis="tohle je na display2";
  mgos_ssd1306_clear(myOled);
  mgos_ssd1306_select_font(myOled,1); 
  mgos_ssd1306_draw_string(myOled,3,3,vypis);
  mgos_ssd1306_draw_string(myOled,3,15,vypis);
